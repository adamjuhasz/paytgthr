...
module App.GetBalance.OneDebitOneCreditSpec
  ( spec
  ) where
...
instance HasPaymentsDB TestMonad where
  getPendingPaymentsOf _ _ _ = return [paymentDebit, paymentCredit]
...
spec :: Spec
spec = parallel $ do
  it "2 payments (debit + credit), credit ignored" $ do
    let limitFor100Score = limitByLevel $ getUserLevelFromScore 100
    trace     <- randomTrace

    liability <- unTestMonad $ getLiability trace (usrUserID userJohn)
    -- credit will be deducted from ledger once it's completed
    liability `shouldBe` (paymentAmount * (-1))

    unTestMonad (getSpendingLimit trace (usrUserID userJohn))
      `shouldReturn` (limitFor100Score + paymentAmount * (-1))

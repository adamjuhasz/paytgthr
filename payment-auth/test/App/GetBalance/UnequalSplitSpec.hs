...
module App.GetBalance.UnequalSplitSpec
  ( spec
  ) where
...
instance HasTransactionsDB TestMonad where
  getPendingTransactionsFor _ _ = return
    [ asos
        { trxSplitAmounts = [ (usrUserID userJohn, 30 % 1)
                            , (usrUserID userJane, 70 % 1)
                            ]
        }
    ]

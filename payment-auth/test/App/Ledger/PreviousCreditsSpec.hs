...
module App.Ledger.PreviousCreditsSpec where
...
instance HasLedgerDB TestMonad where
  ...
  getLedgerJournalType _ (GetPayTgthr _) = return
    [ LedgerJournal { journalId             = JournalId nil
                    ...
                    , journalBalance        = Currency "USD" (-10)
...
instance HasPaymentsDB TestMonad where
  ...
  getPendingPaymentsOf _ theUser _
    | theUser == user1 = return
      [debitPayment { payType = CreditToUser, payAmount = Currency "USD" 10 }]
...
spec :: Spec
spec = parallel $ do
  it "takes into consideration previous credits" $ do
    ...
    fmap (\(PaymentWasCreated Payment {..}) -> (payType, payAmount)) payments
      `shouldBe` [(DebitFromUser, Currency "USD" (20 % 1))]

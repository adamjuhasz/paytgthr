...
module App.Ledger.DebitSpec where
...
spec :: Spec
spec = parallel $ do
  it "works on obvious debit case" $ do
    trace    <- randomTrace
    payments <- unTestMonad $ schedulePaymentsFromLedger trace
    fmap (\(PaymentWasCreated Payment {..}) -> (payType, payAmount, trace))
         payments
      `shouldBe` [(DebitFromUser, Currency "USD" (50 % 1), trace)]

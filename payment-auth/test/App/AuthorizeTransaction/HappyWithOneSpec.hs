...
module App.AuthorizeTransaction.HappyWithOneSpec
  ( spec
  ) where
...
instance HasAccounts TestMonad where
  getRewardsForGroup _ _ = error "Failed to get rewards"

spec :: Spec
spec = parallel $ do
  it "1 person has enough" $ do
    trace <- midToTrace (MessageID nil)
    res   <- unTestMonad $ authorizeTransaction
      trace
      Nothing
      (successTrx { trxSplitAmounts = [(usrUserID userJohn, 100)] })
    res

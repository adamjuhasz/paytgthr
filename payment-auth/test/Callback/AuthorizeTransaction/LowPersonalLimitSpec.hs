...
module Callback.AuthorizeTransaction.LowPersonalLimitSpec
  ( spec
  ) where
...
spec :: Spec
spec = parallel $ do
  describe "Group purchases - sad paths" $ do
    it "One person has a low personal limit, decline transaction" $ do
      trace    <- midToTrace (MessageID nil)
      (res, _) <- unTestMonad $ authorizeNewTransaction
        trace
        (PayWithPrivacy $ PrivacyCardToken "")
        basicTrx { trxDisplayAmount = limitByLevel Level1 * 3 }
